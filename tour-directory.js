...
import { buildDirectoryData, citiesFor, cityName, searchTours, minutes, cleanCopy } from "./tour-directory-data.js";
import { mountStopSlideshow } from "./tour-stop-slideshow.js";
...
function fillStopPanel(panel, tour, stop) {
  ...
  const photoHost = element("div", "stop-photo-host"); panel.append(photoHost);
  activeSlideshow = mountStopSlideshow(photoHost, stop.photos, { name: stop.name, showRightsNote: false });
...
function loadPhotoManifest() {
  if (!photoManifestPromise) photoManifestPromise = (async () => {
    const controller = new AbortController(), timeout = setTimeout(() => controller.abort(), 5000);
    try {
      const response = await fetch("/tour-stop-photos.json", { signal: controller.signal, cache: "no-store" });
...
async function load() {
  ...
  try {
    ...
    const manifestPromise = loadPhotoManifest();
    let response;
    try { response = await fetch("/api/trails", { signal: controller.signal, cache: "no-store" }); }
    finally { clearTimeout(timeout); }
    if (!response.ok) throw new Error("Catalog unavailable");
    const [payload, manifest] = await Promise.all([response.json(), manifestPromise]);
    state.tours = buildDirectoryData(payload, manifest);
...
async function boot() {
  // Preserve bookmarks to the original Seattle booking form and model views.
  const seattleAnchors = new Set(['snStage', 'smStage', 'walkStage', 'ask', 't_name', 't_contact', 't_tour', 't_date', 't_people', 't_note', 'tourMsg']);
  if (seattleAnchors.has(location.hash.slice(1))) {
    location.replace('/tours/seattle' + location.search + location.hash); return;
  }
