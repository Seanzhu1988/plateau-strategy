# Attraction admissions as a revenue line

Target: earn commission on admission tickets to the Metropolitan Museum of Art,
the Empire State Building Observatory, and Top of the Rock, and route the
revenue back into the site.
...
## The honest read before anything else

**These three are the most heavily distributed attractions in New York.** They
already sell through CityPASS, Go City, Viator, GetYourGuide, Klook, Tiqets and
Expedia. They do not have a distribution problem, which means "please give us
commission" is not a proposition they need.

A direct net-rate or commission contract from a venue's trade desk normally
wants demonstrated booking volume, a business entity, insurance, and a sales
history to look at. On the traffic this site has today, a cold approach to
Empire State Realty Trust's trade team gets filed, politely.

That is not a reason to skip it. It is a reason to **do it in the right order**:

1. **Join the affiliate programmes first.** They accept small publishers, need
   no negotiation, and pay from the first booking. This is where the first
...
### A stronger parallel play: Seattle

Worth saying plainly, because it is the same work for a much better hit rate.
This is a **Seattle** company that **drives visitors around Seattle**. To a
Seattle attraction, that is a real and unusual story: *we physically deliver
qualified visitors to your door and can bundle admission into the ride.* To the
Met, a Seattle car service is a stranger with a website.

The Space Needle, Chihuly Garden and Glass, the Museum of Flight, Pike Place
tours and the Woodland Park Zoo are all venues where the trip planner, the
destination book and the actual car service combine into a pitch nobody else
can make. Recommend running that track alongside the New York one, not instead.

---

## What was found, venue by venue
...
answer may well be "go through CityPASS."
...
**The weakest of the three, and it is worth being blunt about why.**

- The Met is a non-profit museum. Admission is **pay-what-you-wish for New York
  State residents** and NY/NJ/CT students. A venue with a pay-what-you-wish
  tier is not optimising ticket distribution the way an observation deck is.
- Its group system is a **reservation** system, not a reseller channel:
  **Group Visit Request Form**,
  `metmuseum.org/visit/group-visits/adult/adult-group-request-form`.
- Groups of **10+** must reserve. **Three weeks** ahead for guided visits, **one
  week** for self-guided. Third-party-led tours require a reservation
  regardless of size.
...
**Read:** approach it as group bookings, not commission. If money comes from
the Met it is likelier to be a tour margin than a ticket commission.

---

## The realistic first revenue: affiliate programmes
...
- Runs through **CJ (Commission Junction)**.
- Reported commission around **4.8%** on general sales, with lower rates on some
  city SKUs and a non-commissionable exclusions list.
- **90-day cookie.**
- Application takes a few days to approve or decline.
- *Rates above are from third-party affiliate directories, not from CityPASS.
  Confirm the current rate card inside CJ before building anything on it.*

**Also worth applying to**, all of which carry all three venues: GetYourGuide,
Viator, Klook, Tiqets. Applying to several is normal and lets you compare what
actually converts.
...
The instinct is to ask for commission. Don't. A trade desk hears that fifty
times a week. Lead with **the visitor you are already moving**, because that is
...
That is true, specific, and it describes intent data rather than raw traffic.

### What not to say

- **Do not overstate the audience.** Invent a number and one look at a traffic
  estimator ends the conversation and your credibility with it. If they ask
  for volume and you have none, say you are starting and ask what threshold
  their programme needs. That question makes you sound like someone who has
  done this before.
- **Do not ask for a bespoke deal.** Ask for their standard trade terms. Easy
...
>
> A meaningful share of those itineraries are New York, and [VENUE] is on them.
> Right now we point people at your site and the referral is worth nothing to
...
One follow-up after five business days, one more after two weeks, then stop and
revisit in a quarter with something new to say. A third chase adds nothing but
a reputation.
...
```bash
BASE=https://plateaustrategy.io      # or http://127.0.0.1:5055 locally

add () { curl -s -b cookies.txt -X POST "$BASE/api/partners/add" \
  -H 'Content-Type: application/json' -d "$1"; echo; }

# sign in first:  curl -s -c cookies.txt -X POST "$BASE/api/owner/login" \
#   -H 'Content-Type: application/json' -d '{"username":"...","password":"..."}'

add '{"name":"Top of the Rock (Rockefeller Center)","type":"attraction",
      "phone":"877-692-7625","website":"https://www.rockefellercenter.com/buy-tickets/group-sales/",
      "address":"45 Rockefeller Plaza, New York, NY",
      "notes":"Group sales desk, 10+ discounted. Also 212-698-2000 ext 3. Email is on the group-sales page - read it off the page. Sells via Klook/GetYourGuide/WAUG/AAA. START HERE: only one of the three with a published phone line."}'

add '{"name":"Empire State Building Observatory","type":"attraction",
      "website":"https://www.esbnyc.com/travel-trade-resource-center",
      "address":"20 W 34th St, New York, NY",
      "notes":"Has a Travel Trade Resource Center. Agents get no discount; trade entry runs through the CityPASS Trade Pass agreement - likely answer is go via CityPASS. Also listed in the NYC Tourism travel-trade directory."}'

add '{"name":"Metropolitan Museum of Art","type":"attraction",
      "website":"https://www.metmuseum.org/visit/group-visits/adult/adult-group-request-form",
